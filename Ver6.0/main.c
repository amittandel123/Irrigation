...
#define ON			1
#define OFF			0
#define OPEN		0 // Reverse logic for relays
#define CLOSE		1 // Reverse logic for relays
...
/* All Valves */
sbit VALVE1	= VALVE_PORT^0;
sbit VALVE2	= VALVE_PORT^1;
sbit VALVE3	= VALVE_PORT^2;
sbit VALVE4	= VALVE_PORT^3;
sbit VALVE5	= VALVE_PORT^4;
sbit VALVE6	= VALVE_PORT^5;
sbit VALVE7	= VALVE_PORT^6;
sbit VALVE8	= VALVE_PORT^7;
...
unsigned int ProgStartd = FALSE,ChangeDisplay = TRUE, ValveSetting = TRUE, WaterSetting = FALSE, DoseSetting = FALSE;
...
void Init()
{
	LCD_PORT = 0x00;	// P1 as O/P port for LCD
	KEY_PORT = 0xFF;	// P0 as i/p port for Keypad
	VALVE_PORT = 0x00;	// P2 as O/P port for VALVES
	P3=0x00;		// P3 as O/P port for Motor
	
	WATER_MOTOR	= CLOSE;	// Water Motor OFF
	DOSE_MOTOR	= CLOSE;	// Chemical Motor OFF
	VALVE_PORT = 0xFF;	// All valves Close
...
void main()
{
	...
	while(1)
	{
		...
		if(ProgStartd)
		{
			...
			for(j = 1; j <= ValveNumber; j++)
			{
				switch(Valve_Sequence[j])
				{
					case 1:
						...
						VALVE_PORT = 0xFE;	// Valve 1 OPEN, rest Closed
						WATER_MOTOR = OPEN;
					...
					case 2:
						...
						Sec_Delay(10);	// WAIT FOR 10 SEC
						VALVE_PORT = 0xFD;	// Valve 2 OPEN, rest Closed
						WATER_MOTOR = OPEN;
						Minute_Delay(InitWaterTime);
						//if(Dose_Sequence[j]>0)
							DOSE_MOTOR = OPEN;
					...
					case 3:
						...
						Sec_Delay(10);	// WAIT FOR 10 SEC
						VALVE_PORT = 0xFB;	// Valve 3 OPEN, rest Closed
						WATER_MOTOR = OPEN;
						Minute_Delay(InitWaterTime);
						//if(Dose_Sequence[j]>0)
							DOSE_MOTOR = OPEN;
					...
					case 4:
						...
						Sec_Delay(10);	// WAIT FOR 10 SEC
						VALVE_PORT = 0xF7;	// Valve 4 OPEN, rest Closed
						WATER_MOTOR = OPEN;
						Minute_Delay(InitWaterTime);
						//if(Dose_Sequence[j]>0)
							DOSE_MOTOR = OPEN;
					...
					case 5:
						...
						Sec_Delay(10);	// WAIT FOR 10 SEC
						VALVE_PORT = 0xEF;	// Valve 5 OPEN, rest Closed
						WATER_MOTOR = OPEN;
						Minute_Delay(InitWaterTime);
						//if(Dose_Sequence[j]>0)
							DOSE_MOTOR = OPEN;
					...
					case 6:
						...
						Sec_Delay(10);	// WAIT FOR 10 SEC
						VALVE_PORT = 0xDF;	// Valve 6 OPEN, rest Closed
						WATER_MOTOR = OPEN;
						Minute_Delay(InitWaterTime);
						//if(Dose_Sequence[j]>0)
							DOSE_MOTOR = OPEN;
					...
					case 7:
						...
						Sec_Delay(10);	// WAIT FOR 10 SEC
						VALVE_PORT = 0xBF;	// Valve 7 OPEN, rest Closed
						WATER_MOTOR = OPEN;
						Minute_Delay(InitWaterTime);
						//if(Dose_Sequence[j]>0)
							DOSE_MOTOR = OPEN;
					...
					case 8:
						...
						Sec_Delay(10);	// WAIT FOR 10 SEC
						VALVE_PORT = 0x7F;	// Valve 8 OPEN, rest Closed
						WATER_MOTOR = OPEN;
						Minute_Delay(InitWaterTime);
						//if(Dose_Sequence[j]>0)
							DOSE_MOTOR = OPEN;
			...
			WATER_MOTOR = CLOSE;
			Sec_Delay(10);	// WAIT FOR 10 SEC
			VALVE_PORT = 0xFF;	// All valves Closed
